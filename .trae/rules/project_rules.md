...
| **代码变更** | 每次代码变更后必须进行**全面测试**,确保功能正常运行,不引入新的Bug | 引入Bug |
| **临时文件** | 临时文件放到 `.trae/temp/{sessionId}` | 污染工作区 |
| **语言规范** | 对话用中文，代码注释用中文 | 不符合规范 |
| **文档优先** | 对于任何操作都**优先查看文档**, 任何修改也需要评估是否维护到文档上 | 不符合规范 |
...
##### 内容测试前置数据准备
内容测试必须重新准备用例数据(历史数据可能无效)，需要根据当前项目工程内容随机生成关于项目多个方面的20个问题，数据格式参考 `data/content-tests.json`，每次覆盖 `data/content-tests.json`。
...
*本文档版本: v3.2 | 最后更新: 2026-03-08 | 面向AI助手*
