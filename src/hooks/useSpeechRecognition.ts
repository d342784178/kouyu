      ...
      if (AudioContext) {
        try {
          ...
          const updateAudioLevel = () => {
            ...
            const rmsLevel = Math.round(rms * 100) // 转换为 0-100 范围
            
            // 同时获取频率数据用于波形显示
            analyser.getByteFrequencyData(frequencyData)
            const freqAverage = frequencyData.reduce((a, b) => a + b, 0) / frequencyData.length
            
            // 使用 RMS 值作为音频电平（夸克浏览器和波形显示都使用这个值）
            setAudioLevel(rmsLevel)
            
            // 调试日志（夸克浏览器）
            if (isQuark) {
              console.log('[useSpeechRecognition] 夸克浏览器音量 - RMS:', rmsLevel, '频率平均:', Math.round(freqAverage))
            }
